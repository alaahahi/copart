...

{
...

    {
        ...
        foreach ($map as $email => $name) {
            ...
            // Balances live on ledger — do not create Wallet rows.
     ...
     * Ensure main cash box exists and return it. Used by payment APIs.
    ...
    public static function idsWithMovements(array $userIds): array
    {
        ...
        // vault_id → legacy_user_id (preferred after wallets drop)
        if (Schema::hasTable('vaults') && Schema::hasColumn('transactions', 'vault_id')) {
            $vaultHits = DB::table('transactions')
                ->join('vaults', 'vaults.id', '=', 'transactions.vault_id')
                ->whereIn('vaults.legacy_user_id', $userIds)
                ->whereNull('transactions.deleted_at')
                ->distinct()
                ->pluck('vaults.legacy_user_id');
            foreach ($vaultHits as $id) {
                $found[(int) $id] = true;
            }
        }

        // Legacy wallet_id join only while wallets table still exists
